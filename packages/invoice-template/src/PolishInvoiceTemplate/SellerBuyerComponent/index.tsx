...
export const SellerBuyerComponent: FunctionComponent<SellerBuyerComponentProps> = ({
  ...
  <Root>
    ...
    <BiggerFontWrapper>
      ...
      {taxId && <LabeledField disableAutoLeftMargin label="NIP:" orientation="horizontal" value={<b>{taxId}</b>} />}
    </BiggerFontWrapper>
    {additionalFields?.map(({ label, value }) => (
      <LabeledField key={label} disableAutoLeftMargin label={label} orientation="horizontal" value={<b>{value}</b>} />
    ))}
  </Root>
)
